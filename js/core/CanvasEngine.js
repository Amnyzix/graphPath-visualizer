...
export class CanvasEngine {
  ...
  undo() {
    if (this.undoStack.length > 0) {
      this.redoStack.push(
        JSON.stringify({ nodes: this.nodes, edges: this.edges, counter: this.nodeCounter })
      );
  ...
  initEvents() {
    ...
    this.svg.addEventListener("mousemove", (e) => {
      ...
      if (this.pendingDrag) {
        ...
        if (Math.hypot(dx, dy) > 6) {
          ...
          try {
            if (this.svg && this.svg.classList) this.svg.classList.add("dragging");
          } catch {
            // swallow errors from unexpected environments
          }
    ...
    window.addEventListener("mouseup", (e) => {
      ...
      const needsRender =
        this.isPanning || this.draggingNode || this.tempEdge || this.isDrawingEdge;

      this.isPanning = false;
      this.draggingNode = null;
      try {
        if (this.svg && this.svg.classList) this.svg.classList.remove("dragging");
      } catch {
        // swallow errors from unexpected environments
      }
      this.isDrawingEdge = false;
  ...
  createNode() {
    console.warn("createNode() doit être implémenté");
  }
  render() {
    console.warn("render() doit être implémenté");
  }
