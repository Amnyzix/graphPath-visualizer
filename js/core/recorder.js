...
export async function recordAndPlay() {
  const player = window.graphPlayer || window.player;
  if (!player || !player.animation || player.animation.length === 0) {
    return alert("Please compile a script first before recording!");
  }

  if (recordingInterval) clearInterval(recordingInterval);

  const svg = ExportManager.getActiveSvg();
  if (!svg) {
    return alert("No active SVG available for recording.");
  }

  const hiddenCanvas = document.getElementById("recorder-canvas");
  if (!hiddenCanvas) {
    return alert("Recorder canvas element not found.");
  }
  ...
  recordingInterval = setInterval(() => {
    ...
    img.onload = () => {
      ...
      if (logDisplay && logDisplay.style.opacity == 1) {
  ...
  const checkDone = setInterval(() => {
    if (player.currentIndex >= player.animation.length - 1) {
