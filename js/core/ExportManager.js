...
export class ExportManager {
  ...
  static async exportGIF(player) {
    ...
    try {
      ...
      const GIF = window.GIF || window.gif;
      if (!GIF) {
        throw new Error(
          "GIF library is not loaded. Please check that gif.js is included before ExportManager."
        );
      }
      ...
      if (btn) btn.innerHTML = originalHtml;
      alert(
        "Une erreur est survenue lors de la création du GIF. Consultez la console (F12) pour plus de détails."
      );
    }
  }
}
