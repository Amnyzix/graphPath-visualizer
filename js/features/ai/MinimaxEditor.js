class MinimaxEditor extends CanvasEngine {
    constructor(svgId) {
        super(svgId);
        this.type = 'ai';
        this.nodeCounter = 1;
    ...
    captureTrace(message = "") {
        ...
        if (this.traces.length > 0) {
            ...
            if (JSON.stringify(lastTrace.nodes) === JSON.stringify(newNodes) &&
    ...
    evaluateNode(node, useAlphaBeta, alpha, beta) {
        ...
        if (node.isLeaf) {
            ...
            this.activeNodeId = null;
            this.captureTrace(`Reached leaf node <b>${node.id}</b>. Returning its score: <b>${node.value}</b>.`);
            return node.value;
        }
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            if (node.isMax) {
                if (childValue > bestValue) {
                    ...
                    this.captureTrace(`<b>${nodeType}</b> node <b>${node.id}</b> updates its value to <b>${bestValue}</b> (found a higher score).`);
                }
                ...
                if (useAlphaBeta) {
                    if (bestValue > alpha) {
                        alpha = bestValue;
                        node.alpha = alpha;
                        this.captureTrace(`Updated <b>Alpha (α)</b> to <b>${alpha}</b> at node <b>${node.id}</b>.`);
                    }
                }
            } else {
                if (childValue < bestValue) {
                    bestValue = childValue;
                    node.computedValue = bestValue;
                    this.captureTrace(`<b>${nodeType}</b> node <b>${node.id}</b> updates its value to <b>${bestValue}</b> (found a lower score).`);
                }
                ...
                if (useAlphaBeta) {
                    if (bestValue < beta) {
                        beta = bestValue;
                        node.beta = beta;
                        this.captureTrace(`Updated <b>Beta (β)</b> to <b>${beta}</b> at node <b>${node.id}</b>.`);
                    }
                }
            }

            if (useAlphaBeta && beta <= alpha) {
                this.captureTrace(`<span style="color: #EF4444; font-weight: bold;">Pruning triggered!</span> Beta (${beta}) is ≤ Alpha (${alpha}). No need to explore further.`);
        ...
        node.isEvaluating = false;
        this.activeNodeId = null;
        this.captureTrace(`Finished evaluating <b>${nodeType}</b> node <b>${node.id}</b>. Final choice: <b>${bestValue}</b>.`);
