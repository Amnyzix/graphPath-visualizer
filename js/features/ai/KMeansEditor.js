...
export class KMeansEditor extends CanvasEngine {
  constructor(svgId) {
    super(svgId);
    this.type = "kmeans";
    this.points = [];
  ...
  runKMeans() {
    ...
    while (hasChanged && iterations < maxIterations) {
      ...
      this.captureTrace(
        "<b>Assignment Step:</b> Each point is linked to its nearest centroid based on Euclidean distance."
      );
      ...
      if (hasChanged) {
        this.captureTrace(
          "<b>Update Step:</b> Centroids move to the exact average center (mean) of all points currently assigned to them."
        );
      }
    ...
    this.captureTrace(
      "<b>Convergence:</b> Centroids no longer move. The K-Means algorithm is complete!"
    );
  ...
  captureTrace(message = "") {
    ...
    if (this.traces.length > 0) {
      ...
      if (
        JSON.stringify(lastTrace.points) === JSON.stringify(currentPoints) &&
  ...
  render() {
    ...
    this.points.forEach((p) => {
      ...
      const color =
        p.cluster !== null ? this.clusterColors[p.cluster % this.clusterColors.length] : "#94A3B8";
