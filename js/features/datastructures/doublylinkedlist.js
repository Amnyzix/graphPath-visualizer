...
class DoublyLinkedList {
  ...
  async append(value) {
    ...
    this.updateStatus(
      `Successfully appended ${value}. Both 'next' and 'prev' pointers set.`,
      "success"
    );
...
export async function appendDLLNode() {
...
export async function deleteDLLNode() {
  const input = document.getElementById("ds-value-input");
  const val = parseInt(input.value);
  if (!isNaN(val) && !doublyLinkedListEngine.isAnimating) {
...
export function clearDLLDataStructure() {
  ...
  doublyLinkedListEngine.updateStatus(
    "Doubly Linked List cleared. Enter a value to start.",
    "header"
  );
}
