...
class BSTAlgorithms {
    ...
    static search(root, value) {
        ...
        if (!root) {
            animation.addFrame(this.createFrame(root, null, `The tree is empty. Value ${value} not found.`, DSColors.error));
            return animation;
        }

        while (current !== null) {
            animation.addFrame(this.createFrame(root, current.value, `Checking node ${current.value}...`));

            if (value === current.value) {
                animation.addFrame(this.createFrame(root, current.value, `Target found! ${value} is in the tree.`, {fill: 'var(--visited-fill)', stroke: 'var(--visited-stroke)'}));
                return animation;
            } else if (value < current.value) {
                animation.addFrame(this.createFrame(root, current.value, `${value} < ${current.value}. Searching left.`));
                current = current.left;
            } else {
                animation.addFrame(this.createFrame(root, current.value, `${value} > ${current.value}. Searching right.`));
                current = current.right;
            }
        }

        animation.addFrame(this.createFrame(root, null, `Reached a dead end. Value ${value} not found.`, DSColors.error));
        return animation;
    }
    ...
    static insert(root, value) {
        ...
        if (!currentRoot) {
            ...
            animation.addFrame(this.createFrame(currentRoot, value, `Tree empty. ${value} becomes root.`, DSColors.visited));
            return { animation, newRoot: currentRoot }; // On renvoie aussi la nouvelle racine pour MAJ le document
        }

        let curr = currentRoot;
        while (true) {
            animation.addFrame(this.createFrame(currentRoot, curr.value, `Comparing ${value} with ${curr.value}`));

            if (value === curr.value) {
                animation.addFrame(this.createFrame(currentRoot, curr.value, `Value ${value} exists!`, DSColors.error));
                break;
            } else if (value < curr.value) {
                if (!curr.left) {
                    curr.left = { value, left: null, right: null, x:0, y:0 };
                    animation.addFrame(this.createFrame(currentRoot, value, `Inserted ${value} to the left.`, DSColors.visited));
                    break;
                }
                curr = curr.left;
            } else {
                if (!curr.right) {
                    ...
                    animation.addFrame(this.createFrame(currentRoot, value, `Inserted ${value} to the right.`, DSColors.visited));
                    break;
                }
                curr = curr.right;
            }
    ...
    static delete(root, value) {
        ...
        while (current !== null) {
            animation.addFrame(this.createFrame(tree, current.value, `Searching for ${value}... Checking ${current.value}.`));
            if (value === current.value) {
                found = true;
        ...
        if (!current.left || !current.right) {
            ...
            animation.addFrame(this.createFrame(tree, current.value, `2 children found. Finding successor.`));
            let successorParent = current;
            let successor = current.right;
            while (successor.left) {
                successorParent = successor;
                successor = successor.left;
                animation.addFrame(this.createFrame(tree, successor.value, `Moving to successor: ${successor.value}`));
            }
            current.value = successor.value;
            if (successorParent.left === successor) successorParent.left = successor.right;
            else successorParent.right = successor.right;
            animation.addFrame(this.createFrame(tree, current.value, `Value updated and successor removed.`));
        }
        return { animation, newRoot: tree };
    }
    ...
    static findMin(root) {
        ...
        while (current && current.left) {
            animation.addFrame(this.createFrame(root, current.value, `Moving left to ${current.left.value}...`));
            current = current.left;
        }
        if (current) animation.addFrame(this.createFrame(root, current.value, `Minimum found: ${current.value}`, DSColors.visited));
        return animation;
    }
    ...
    static findMax(root) {
        ...
        while (current && current.right) {
            animation.addFrame(this.createFrame(root, current.value, `Moving right to ${current.right.value}...`));
            current = current.right;
        }
        if (current) animation.addFrame(this.createFrame(root, current.value, `Maximum found: ${current.value}`, DSColors.visited));
        return animation;
    }
    ...
    static traverse(root, type) {
        ...
        animation.addFrame(this.createFrame(currentRoot, null, `Starting ${type} traversal...`, DSColors.current, currentSequence, type));

        fullOrder.forEach((node, i) => {
            currentSequence.push(node.value);
            animation.addFrame(this.createFrame(currentRoot, node.value, `Visiting ${node.value} (${i+1}/${fullOrder.length})`, DSColors.visited, currentSequence, type));
        });

        animation.addFrame(this.createFrame(currentRoot, null, `Traversal Complete! Processed ${fullOrder.length} nodes.`, DSColors.visited, currentSequence, type));
