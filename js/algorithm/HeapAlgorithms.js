...
import { DSColors } from "./BSTAlgorithms.js";

export class HeapAlgorithms {
  ...
  static insert(heapState, type, value) {
    ...
    animation.addFrame(
      this.createFrame(heap, [], `Starting insertion of ${value} into ${typeName}...`)
    );
    ...
    animation.addFrame(
      this.createFrame(
        ...
        `Inserted ${value} at the bottom of the heap.`
      )
    );
  ...
  static extractRoot(heapState, type) {
    ...
    animation.addFrame(
      this.createFrame(
        heap,
        [{ index: 0, theme: DSColors.error }],
        `Removing the root node (${rootVal}).`
      )
    );
    ...
    if (heap.length > 0) {
      ...
      animation.addFrame(
        this.createFrame(
          heap,
          [{ index: 0, theme: DSColors.current }],
          `Moved last element (${lastVal}) to root.`
        )
      );
        ...
        if (target !== currentIndex) {
          ...
          animation.addFrame(
            this.createFrame(
              heap,
              [{ index: currentIndex, theme: DSColors.visited }],
              `Node satisfies heap property. Restoration complete!`
            )
          );
          break;
        }
  ...
  static search(heapState, type, value) {
    ...
    for (let i = 0; i < heap.length; i++) {
      animation.addFrame(
        this.createFrame(
          heap,
          [{ index: i, theme: DSColors.current }],
          `Checking index ${i} (value: ${heap[i]})...`
        )
      );
      ...
      if (heap[i] === value) {
        ...
        animation.addFrame(
          this.createFrame(
            heap,
            [{ index: i, theme: DSColors.visited }],
  ...
  static delete(heapState, type, value) {
    ...
    for (let i = 0; i < heap.length; i++) {
      animation.addFrame(
        this.createFrame(heap, [{ index: i, theme: DSColors.current }], `Searching for ${value}...`)
      );
      if (heap[i] === value) {
        targetIndex = i;
        animation.addFrame(
    ...
    animation.addFrame(
      this.createFrame(
        heap,
        [{ index: targetIndex, theme: DSColors.current }],
        `Removed ${removedValue} from the heap.`
      )
    );
      ...
      if (targetIndex > 0 && this.compare(heap[targetIndex], heap[parentIndex], type)) {
        animation.addFrame(
          this.createFrame(
            heap,
            [{ index: targetIndex, theme: DSColors.current }],
            `Element might be too prioritized. Bubbling up...`
          )
        );
      ...
      // Sinon, on la redescend (Bubble Down)
        ...
        animation.addFrame(
          this.createFrame(
            heap,
            [{ index: targetIndex, theme: DSColors.current }],
            `Element might violate children. Bubbling down...`
          )
        );
