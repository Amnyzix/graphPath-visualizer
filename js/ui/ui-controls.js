import { updateEditorThemes } from "./editor.js";

const themeToggle = document.getElementById("theme-toggle");

const getActiveGraphEditor = () => {
  if (window.activeEditor) return window.activeEditor;
  if (window.AppRegistry) return window.AppRegistry.get("graphs");
  return window.graphsApp || null;
};

const renderEditor = () => {
  const editor = getActiveGraphEditor();
  if (editor && typeof editor.render === "function") editor.render();
};

const deleteSelectedNodes = (nodeSet) => {
  const editor = getActiveGraphEditor();
  if (!editor) return;
  ...
  if (!editor.document || !nodeSet) return;
  nodeSet.forEach((id) => editor.document.removeNode(id));
  if (editor.selectedNodes && typeof editor.selectedNodes.clear === "function") {
...
export function deleteNodeFromMenu() {
  if (!contextNodeId) return;
  const editor = getActiveGraphEditor();
  if (!editor) return;

  if (editor.selectedNodes.has(contextNodeId) && editor.selectedNodes.size > 1) {
    deleteSelectedNodes(editor.selectedNodes);
  } else {
    deleteSelectedNodes(new Set([contextNodeId]));
  }

  document.getElementById("context-menu").style.display = "none";
}

export function deleteEdgeFromMenu() {
  if (!contextEdge) return;
  const editor = getActiveGraphEditor();
  if (!editor || !editor.document) return;

  editor.document.edges = editor.document.edges.filter((e) => e !== contextEdge);
  contextEdge = null;
  editor.saveState();
  editor.render();
}
  ...
  if (e.ctrlKey) {
    switch (e.key.toLowerCase()) {
      case "z":
        ...
        else if (window && typeof window.undo === "function") window.undo(); // Fallback Legacy
        break;

      case "y":
        e.preventDefault();
        if (editor && typeof editor.redo === "function") editor.redo();
        else if (window && typeof window.redo === "function") window.redo(); // Fallback Legacy
        break;
    ...
    switch (e.key) {
      ...
      case "Backspace":
        if (editor && typeof editor.deleteSelected === "function") {
          editor.deleteSelected();
        } else if (editor && editor.selectedNodes && editor.selectedNodes.size > 0) {
          deleteSelectedNodes(editor.selectedNodes);
        }
        break;
    }
...
if (splitter && leftPanel && rightPanel) {
  ...
  document.addEventListener("mousemove", (e) => {
    ...
    if (newWidth >= 250 && newWidth <= 600) {
      ...
      if (window.galEditor && typeof window.galEditor.refresh === "function") {
        window.galEditor.refresh();
      }
    }
  });
...
window.switchAiAlgorithm = switchAiAlgorithm;
window.renameNode = renameNode;
window.deleteNodeFromMenu = deleteNodeFromMenu;
window.deleteEdgeFromMenu = deleteEdgeFromMenu;
...
export function switchTab(tabName) {
  ...
  if (tabName === "script") {
    ...
    if (window.galEditor && typeof window.galEditor.refresh === "function") {
      setTimeout(() => window.galEditor.refresh(), 10);
    }
...
export function switchAiAlgorithm(algo) {
  ...
  if (algo === "minimax") {
    ...
    paramSlider.min = 2;
    paramSlider.max = 6;
    paramSlider.onchange = function () {
      window.aiApp.generateTree(this.value);
    };
  } else if (algo === "kmeans") {
    ...
    paramSlider.max = 10;
    paramSlider.onchange = function () {
      /* K-Means lira cette valeur au clic sur Run */
    };
  } else if (algo === "knn") {
    ...
    paramSlider.onchange = function () {
      /* K-NN lira cette valeur au clic sur Run */
    };
  }
}
