...
async function testAutomataWord() {
...
function generateAutomatonFromRegex() {
...
function removeEpsilonFromAutomaton() {
    if (!automataApp || automataApp.nodes.length === 0) return;
...
function determinizeAutomaton() {
    ...
    if (hasEpsilon) {
        alert("Attention : Cet algorithme s'applique sur un NFA sans epsilon-transitions. Veuillez d'abord cliquer sur 'Remove ε-transitions'.");
        return;
    }
...
function switchAutomataTab(tabId, clickedBtn) {
